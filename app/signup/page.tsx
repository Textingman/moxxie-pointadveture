      ...
      <section className="py-12" style={{ backgroundColor: '#F5F7FA' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl font-extrabold mb-4" style={{ color: '#1F2937' }}>
            Start Your Points Adventure — Free
          </h1>
          <p className="text-xl max-w-2xl mx-auto" style={{ color: '#6B7280' }}>
            Get AI-powered guidance on earning and maximizing points — no sales calls, no pressure.
          </p>
        </div>
      </section>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12" style={{ border: '1px solid #E5E7EB' }}>
          {submitStatus === 'success' ? (
            <div className="text-center py-12">
              ...
              <Link href="/" className="inline-block px-6 py-3 text-white font-semibold rounded-lg transition-opacity hover:opacity-90" style={{ backgroundColor: '#10B981' }}>
                Return to Home
              </Link>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4 pt-2">
                <div className="flex items-start gap-3">
                  <input type="checkbox" id="accountNotifications" checked={accountNotifications} onChange={(e) => setAccountNotifications(e.target.checked)} className="mt-1 w-4 h-4 rounded" style={{ accentColor: '#10B981' }} />
                  <label htmlFor="accountNotifications" className="text-sm" style={{ color: '#6B7280' }}>
                    By checking this box, I agree to receive Account Notification SMS messages from Point Adveture at the phone number provided. I understand I may opt out at any time by replying STOP. Reply HELP for help. Message and data rates may apply. Message frequency varies. Opting in is optional and not required to submit this form. Consent is not required as a condition of purchasing any products or services. See our <a href="/privacy" style={{ color: '#10B981' }}>Privacy Policy</a> and <a href="/terms" style={{ color: '#10B981' }}>Terms of Service</a>.
                  </label>
                </div>
                <div className="flex items-start gap-3">
                  <input type="checkbox" id="customerCare" checked={customerCare} onChange={(e) => setCustomerCare(e.target.checked)} className="mt-1 w-4 h-4 rounded" style={{ accentColor: '#10B981' }} />
                  <label htmlFor="customerCare" className="text-sm" style={{ color: '#6B7280' }}>
                    By checking this box, I agree to receive Customer Care SMS messages from Point Adveture at the phone number provided. I understand I may opt out at any time by replying STOP. Reply HELP for help. Message and data rates may apply. Message frequency varies. Opting in is optional and not required to submit this form. Consent is not required as a condition of purchasing any products or services. See our <a href="/privacy" style={{ color: '#10B981' }}>Privacy Policy</a> and <a href="/terms" style={{ color: '#10B981' }}>Terms of Service</a>.
                  </label>
                </div>
              </div>

              <button type="submit" disabled={isSubmitting} className="w-full py-4 text-white text-lg font-semibold rounded-lg transition-opacity hover:opacity-90" style={{ backgroundColor: isSubmitting ? '#9CA3AF' : '#10B981' }}>
                {isSubmitting ? 'Submitting...' : 'Start Your Adventure'}
              </button>
            </form>
          )}
        </div>
